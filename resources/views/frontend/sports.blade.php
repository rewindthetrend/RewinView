          ...
          @foreach ($sports as $post)
            ...
            <div class="item px-2">
                <div class="fh5co_hover_news_img">
                    <div class="fh5co_news_img"><img src="images/{{ ($post->image) }}" width="324" height="235" alt="img"/></div>
                    <div>
                        <a href="{{ route('single',[$post->id]) }}" class="d-block fh5co_small_post_heading"><span class="">{{ substr($post->description,0 ,50) }}{{ strlen($post->description)> 50 ? "...": ""}}</span></a>
                        <div class="c_g"><i class="fa fa-clock-o"></i>{{ date('M j, Y', strtotime($post->created_at)) }}</div>
                    </div>
                </div>
            </div>
            <?php $count++; ?>
